...
// ### jiho
static int generate_ppv2_tlv_header(struct list *tlv_nodes, char *buf, int max_len) {
	struct tlv_node *node;
	int len = 0, tlv_val_len;

	list_for_each_entry(node, tlv_nodes, list) {
		tlv_val_len = get_tlv_length(&node->tlv);

		ha_notice("### append tlv header from remote: node=%p, type=0x%x, len=%d, val=%s \n", 
				  node, node->tlv.type, tlv_val_len, (char*)node->tlv.value);

		if (max_len <= (len+tlv_val_len+5)) {
			break;
		}

		len += snprintf(&buf[len], max_len-len, "0x%02x=", node->tlv.type);
		len += a2base64((char*)node->tlv.value, tlv_val_len, &buf[len], max_len-len);
		buf[len++] = ';';
	}
					      ...
					      struct session *sess, struct stream *s, int flags)
{
	...
	if (srv != NULL) {
		len += generate_ppv2_tlv_header(&srv->tlv_nodes, &val[len], max_len - len);
	}

	if (conn != NULL) {
		len += generate_ppv2_tlv_header(&conn->tlv_nodes, &val[len], max_len - len);
	}
	...
	ha_notice("### set ppv2-tlv-header: n=%s, v=%s \n", istptr(n), istptr(v));
					      ...
					      struct session *sess, struct stream *s, int flags)
{
	...
	ha_notice("### set ppv2-header: n=%s, v=%s \n", istptr(n), istptr(v));
					   ...
					   struct act_rule *rule, char **err)
{
	...
	if (args[*orig_arg-1][13] == 'h') {
		ha_notice("### parse set_ppv2_header \n");
	...
	// set-proxy-v2-tlv-header
		...
		ha_notice("### parse set_ppv2_tlv_header \n");
...
static struct action_kw_list http_req_actions = {
	.kw = {
		...
		// jiho
		{ "set-proxy-v2-header",     parse_http_set_ppv2_header, 0 },
		{ "set-proxy-v2-tlv-header", parse_http_set_ppv2_header, 0 },
